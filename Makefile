...
# Creates a kapi cluster using kind then runs the kapi-example binary locally which connects to it.
.PHONY: example-local
...
# Lists the images in the kapi cluster's internal container registry
.PHONY: example-cluster-images
example-cluster-images:
	@docker exec -it ${KAPI_CLUSTER}-control-plane crictl images
